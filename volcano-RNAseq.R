...
volcanoplot <- function (res, lfcthresh=2, sigthresh=0.01, main="Volcano Plot", legendpos="bottomright", labelsig=FALSE, textcx=1, ...) {
  ...
  legend(legendpos, xjust=0.6, yjust=0.6, legend=c(paste("down-regulated"), paste("not significant"), "up-regulated"), pch=15, col=c("blue","grey","red"))
}
png("diffexpr-volcanoplot.png", 1500, 1500, pointsize=40)
volcanoplot(res, lfcthresh=0, sigthresh=0.01, textcx=.8, xlim=c(-12, 12))
dev.off()
